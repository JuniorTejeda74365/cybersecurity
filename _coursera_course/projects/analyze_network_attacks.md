...
This attack affected the organization’s network by degrading the availability and performance of the website. Legitimate HTTP requests were either delayed or dropped, causing slow page loads, repeated connection retries, and HTTP errors such as 504 Gateway Time-out. Although the web server was still reachable, it could not reliably maintain connections due to resource exhaustion, which disrupted normal website functionality.
